...
- (NSString *)genPackage
{
    ...
    NSString* paymoney = [NSString stringWithFormat:@"%.0f",[money doubleValue]*100];
    
    NSLog(@"订单号是：%@-----支付金额是：%@---微信金额：%@",oderid,money,paymoney);
    
    
    // 构造参数列表
    NSMutableDictionary *params = [NSMutableDictionary dictionary]; 
    [params setObject:@"WX" forKey:@"bank_type"];  //银行类型  保持WX不变
    [params setObject:@"爱慕商城手机端订单详情" forKey:@"body"];
    [params setObject:@"1" forKey:@"fee_type"];  //金支付币种,目前只支 持人民币,默认值是 1-人 民币
    [params setObject:@"UTF-8" forKey:@"input_charset"];
    //lee999 微信回调地址"http://m.aimer.com.cn/shopcart/confim_wxpay"  WX_notifyURL
    [params setObject:WX_notifyURL forKey:@"notify_url"];
    [params setObject:oderid forKey:@"out_trade_no"]; //订单号  //[self genOutTradNo]
    [params setObject:WXPartnerId forKey:@"partner"];
    [params setObject:[CommonUtil getIPAddress:YES] forKey:@"spbill_create_ip"]; 
    [params setObject:paymoney forKey:@"total_fee"];    // 1 =＝ ¥0.01
...
- (NSMutableData *)getProductArgs
{
    ...
    NSMutableDictionary *params = [NSMutableDictionary dictionary]; 
    [params setObject:kWeiXinKey forKey:@"appid"];
...
- (void)getAccessToken
{
    NSString *getAccessTokenUrl = [NSString stringWithFormat:@"https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=%@&secret=%@", kWeiXinKey, WXAppSecret];
    ...
    WXPayClient *weakSelf = self;
    ASIHTTPRequest *weakRequest = self.request;
...
- (void)getPrepayId:(NSString *)accessToken
{
    ...
    [self.request setCompletionBlock:^{
        ...
        if (prePayId) {
            ...
            request.timeStamp = (unsigned int)[weakSelf.timeStamp longLongValue];
