...
+ (void)initialize
{
    if (self == [BlockAlertView class])
    {
        ...
        background = [[background stretchableImageWithLeftCapWidth:30 topCapHeight:kAlertViewBackgroundCapHeight] retain];
...
- (void)show
{
    ...
    for (NSUInteger i = 0; i < _blocks.count; i++)
    {
        ...
        if (i == 0) {
            ...
            UIImageView *lineImage = [[UIImageView alloc] initWithFrame:CGRectMake(70,kAlertButtonHeight-12, width-140, 1)];
            [lineImage setBackgroundColor:[UIColor blueColor]];
            ...
            if (button.tag  == 3) {
                ...
                [button setTitleColor:kAlertViewButtonTextColor forState:UIControlStateNormal];
                [button setTitleColor:kAlertViewButtonShadowColor forState:UIControlStateHighlighted];
            }
            [button setTitle:title forState:UIControlStateNormal];
            button.accessibilityLabel = title;
